  ...
  return (
    <div className="min-h-screen">
      <main className="pt-24">
        ...
        {/* Hero: DynamicHero-inspired (no shared imports) */}
            ...
            <div className="flex items-start justify-between gap-6">
              <div>
                ...
                <p className="mt-2 text-lg text-[color:var(--muted-foreground)] max-w-2xl">
                  Clear for non‑technical teams. Deep enough for engineers. Tap
                  or hover a card to see how we approach delivery, what you get,
                  and when.
                </p>
              </div>
              ...
              <div className="lg:col-span-8 space-y-4">
                {items.map((item) => {
                  ...
                  return (
                    <div
                      key={item.title}
                      className="relative group rounded-xl border border-[color:var(--border)] bg-[color:var(--surface)]/70 backdrop-blur supports-[backdrop-filter]:bg-[color:var(--surface)]/60 transition-all duration-300 group-hover:border-transparent hover:shadow-lg hover:shadow-orange-500/20"
                    >
                      {/* Hidden checkbox to enable tap-to-expand on mobile */}
                      <input id={id} type="checkbox" className="peer sr-only" />

                      {/* Overlay for emerald gradient on hover or when checked */}
                      <div
                        aria-hidden
                        className="pointer-events-none absolute inset-0 rounded-xl opacity-0 transition-opacity duration-300 group-hover:opacity-100 peer-checked:opacity-100 bg-gradient-to-tr from-emerald-700 to-emerald-900"
                      />

                      {/* Header (hover/tap target) */}
                      <label
                        htmlFor={id}
                        className="relative z-10 flex w-full cursor-pointer items-start justify-between gap-4 p-5"
                        aria-controls={contentId}
                      >
                        <div className="min-w-0">
                          <h4 className="text-lg md:text-xl font-semibold text-[color:var(--foreground)] group-hover:text-white peer-checked:text-white">
                            {item.title}
                          </h4>
                          <p className="mt-1 text-base text-[color:var(--muted-foreground)] group-hover:text-white/90 peer-checked:text-white/90">
                            {c.blurb}
                          </p>
                        </div>
                        <div className="shrink-0 h-9 w-9 rounded-full bg-gradient-to-tr from-emerald-500 to-emerald-700 grid place-items-center text-slate-900 transition-transform duration-300 group-hover:rotate-180 peer-checked:rotate-180 group-hover:bg-white/20 peer-checked:bg-white/20 group-hover:text-white peer-checked:text-white">
                          <svg
                            width="18"
                            height="18"
                            viewBox="0 0 24 24"
                      ...
                      <div
                        id={contentId}
                        className="relative z-10 grid transition-[grid-template-rows] duration-300 [grid-template-rows:0fr] group-hover:[grid-template-rows:1fr] peer-checked:[grid-template-rows:1fr]"
                      >
                        <div className="overflow-hidden px-5">
                          <ul className="mt-2  grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {c.bullets.map((b) => (
                              <li
                                key={b}
                                className="flex items-start gap-2 text-base text-[color:var(--foreground)]/90 group-hover:text-white peer-checked:text-white"
                              >
                          ...
                          <div className="mt-4 flex flex-wrap gap-2 pb-5 text-sm">
                            <span className="rounded-md border px-2.5 py-1 border-[color:var(--border)] bg-[color:var(--surface)] text-[color:var(--muted-foreground)] group-hover:border-white/20 group-hover:bg-white/10 group-hover:text-white peer-checked:border-white/20 peer-checked:bg-white/10 peer-checked:text-white">
                              Best for: {c.bestFor}
                            </span>
                            <span className="rounded-md border px-2.5 py-1 border-[color:var(--border)] bg-[color:var(--surface)] text-[color:var(--muted-foreground)] group-hover:border-white/20 group-hover:bg-white/10 group-hover:text-white peer-checked:border-white/20 peer-checked:bg-white/10 peer-checked:text-white">
                              Timeline: {c.timeline}
